...
enum ChatLineFlag
{
	// CHATLINE_FIRST only used for ChatFormattedLine:
	// set if first formatted line of one ChatLine
	CHATLINE_FIRST = 0x1,
	// was message generated by server?
	CHATLINE_SERVER = 0x2,
	// was message written by self?
	CHATLINE_SELF = 0x4,
	// is own nick highlighted by this line?
	CHATLINE_HIGHLIGHT = 0x8,
};
...
struct ChatLine
{
	...
	ChatLine(std::wstring a_name, std::wstring a_text, u32 a_flags):
		age(0.0),
		name(a_name),
		text(a_text),
		flags(a_flags)
...
struct ChatFormattedLine
{
	...
	// Flags of corresponding ChatLine
	// plus CHATLINE_FIRST if first formatted line of the ChatLine
	u32 flags;
};
...
public:
	...
	ChatBuffer& getRecentBuffer(bool filtered);
	// Concatenate all recent messages
	std::wstring getRecentChat(bool filtered);
...
private:
	...
	ChatBuffer m_recent_filtered_buffer;
	ChatPrompt m_prompt;
